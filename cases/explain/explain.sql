...
explain select userID,MAX(score) max_score from t3 where userID <2 || userID > 3 group by userID order by max_score;
explain select userID, userID DIV 2 as user_dir, userID%2 as user_percent, userID MOD 2 as user_mod from t3 where userID > 3 ;
