...
def parse_log_file(log_file):
    """
    解析日志文件，提取文件路径和执行时间
    返回: (file_times, dir_times)
    - file_times: {文件路径: 总耗时}
    - dir_times: {目录路径: 总耗时}
    """
    file_times = defaultdict(float)
    dir_times = defaultdict(float)
    
    # 匹配模式: "has been executed, and cost: X.XXXs"
    pattern = r'The script file\[([^\]]+)\] has been executed, and cost: ([\d.]+)s'
...
def main():
    ...
    file_times, dir_times = parse_log_file(log_file)
    
    print(f"\n解析完成!")
    print(f"共找到 {len(file_times)} 个文件, {len(dir_times)} 个目录")
    
    # 打印最耗时的文件
    print_top_items(file_times, "最耗时的测试文件", top_n=30)
    
    # 打印最耗时的目录
    print_top_items(dir_times, "最耗时的测试目录", top_n=30)
